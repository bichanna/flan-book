# `std`
`std` module defines basic functions for working with Flan values and functions, iterators, and control flow.
...
## `clone(x)`
`clone` takes any Flan value and produces a shallow copy of it that will not mutate if the original mutates.
```js
list := [1, 2, 3]
new_list := clone(list)
